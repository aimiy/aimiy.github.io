...
组件比较简单，没有很复杂的交互，因此index.js只需要引入模板即可
...
除了vue组件外，还有类似于toast之类的全局事件，这类调用的时候不是直接从组件库import，是直接在this上调用事件。此时vue模板是作为模板，跟随事件实时生成实例的。
先写一个toast的显示框
...
全局事件的注册就不是直接export vue组件了，而是js，说白了其实就是export一个方法，组件只是一个用来显示的模板。
